...
const UserManagement = () => {
    ...
    const handleDelete = async (userToDelete) => {
        const result = await Swal.fire({
            title: 'Are you sure?',
            text: `This will delete ${userToDelete.name}!`,
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire('Deleted!', `${userToDelete.name} has been deleted.`, 'success');
            } catch (err) {
                console.error(err);
                        ...
                        {users.map((u) => (
                            <tr key={u._id} className="text-center border-b hover:bg-gray-50">
                                <td className="py-2 px-4">
                                    <img src={u.image || '/default-profile.png'} alt={u.name} className="w-10 h-10 rounded-full mx-auto"/>
                                </td>
                                ...
                                <td className="py-2 px-4 flex justify-center gap-2">
                                    <button onClick={() => setEditingUser(u)} className="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded flex items-center gap-1">
                                        <FaEdit /> Edit
                                    </button>
                                    <button onClick={() => handleDelete(u)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded flex items-center gap-1">
                                        <FaTrash /> Delete
                                    </button>
                                </td>
                            </tr>
